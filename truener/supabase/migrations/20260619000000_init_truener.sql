...
comment on table public.users is 'Truenerのユーザー基本情報。auth.usersと1対1で対応する。';
...
-- 候補一覧閲覧: 年齢確認済み・アクティブな異性ユーザーを閲覧可能
...
-- 審査結果の更新はサービスロール（管理者）のみ
-- UPDATE はアプリサーバーから service_role キーで実行
...
-- マッチングレコードの作成はサービスロールから（いいね承認トリガー経由）
-- is_first_message_sent の更新は当事者が可能
